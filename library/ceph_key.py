...
def generate_ceph_authtool_cmd(cluster, name, secret, caps, auid, dest, containerized=None):
    ...
    file_destination = os.path.join(dest + "/" + cluster + "." + name + ".keyring")
...
def create_key(module, result, cluster, name, secret, caps, import_key, auid, dest, containerized=None):
    ...
    cmd_list.append(generate_ceph_authtool_cmd(cluster, name, secret, caps, auid, dest, containerized))
...
def run_module():
    ...
    if state == "present":
        ...
        rc, cmd, out, err = exec_commands(module, create_key(module, result, cluster, name, secret, caps, import_key, auid, dest, containerized))
